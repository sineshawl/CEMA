...
plt <- ggplot(model_plot)+
  geom_line(aes(time, value, col=name), linewidth=1.5)+
  scale_x_continuous("Time", breaks = seq(0, 365, by=30))+
  scale_y_continuous("Population", breaks = seq(0, 500, by=10))+
...
# write a function to compute incidence:
# the prameters: num_cases, population

incidence <- function(num_cases, population){
  return(num_cases/ population)
}
incidence(50, 100)
